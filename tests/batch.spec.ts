...
import { Id, RmrkError } from "types-returns/rmrk_example_equippable";
...
describe("RMRK Batch calls test", () => {
  ...
  it("deployer mints and transfers many works", async () => {
    const PARENT_TOKEN_IDS: ArgumentTypes.Id[] = Array.from({ length: 50 }, (_, i) => ({ u64: i + 1 }));
    const CHILD_TOKEN_IDS: ArgumentTypes.Id[] = Array.from({ length: 50 }, (_, i) => ({ u64: i + 1 }))
    const ASSET_ID1 = 1;
    ...
    for (let i = 1; i <= CHILD_TOKEN_IDS.length; i++) {
    ...
    for (let i = 1; i <= PARENT_TOKEN_IDS.length; i++) {
    ...
    const manyUsers = createBatchUsers(PARENT_TOKEN_IDS.length);
  ...
  it("Set of batch calls works", async () => {
    const PARENT_TOKEN_IDS: ArgumentTypes.Id[] = Array.from({ length: 100 }, (_, i) => ({ u64: i + 1 }));
    const CHILD_TOKEN_IDS: ArgumentTypes.Id[] = Array.from({ length: 100 }, (_, i) => ({ u64: i + 1 }))
    const ASSET_ID1 = 1;
    const sets = 3;
    ...
    for (let i = 0; i < sets; i++) {
      let next_ids = PARENT_TOKEN_IDS.map(id => ({ u64: id.u64 as any + (i * 100) }))
      await mintMany(parent, deployer, next_ids);
    }

    expect(
      (await parent.query.totalSupply()).value.unwrap().toNumber()
    ).to.equal(PARENT_TOKEN_IDS.length * sets);
    expect((await parent.query.ownerOf({ u64: 1 })).value.unwrap()).to.equal(
      deployer.address
    );
    ...
    // deployer mints many child tokens
    for (let i = 0; i < sets; i++) {
      let next_ids = CHILD_TOKEN_IDS.map(id => ({ u64: id.u64 as any + (i * 100) }))
      await mintMany(child, deployer, next_ids);
    }
    expect(
      (await child.query.totalSupply()).value.unwrap().toNumber()
    ).to.equal(CHILD_TOKEN_IDS.length * sets);
    expect((await child.query.ownerOf({ u64: 1 })).value.unwrap()).to.equal(
      deployer.address
    );
    ...
    for (let i = 0; i < sets; i++) {
      ...
      for (let j = CHILD_TOKEN_IDS.length * i + 1; j <= CHILD_TOKEN_IDS.length * (i + 1); j++) {
    ...
    for (let i = 1; i <= CHILD_TOKEN_IDS.length * sets; i++) {
      expect(
        (await child.query.totalTokenAssets({ u64: i }))?.value.unwrap().ok.toString()
      ).to.be.equal("1,0");
    }
    ...
    for (let i = 0; i < sets; i++) {
      ...
      for (let j = PARENT_TOKEN_IDS.length * i + 1; j <= PARENT_TOKEN_IDS.length * (i + 1); j++) {
    ...
    // check that all parent tokens have 1 child
    for (let i = 1; i <= PARENT_TOKEN_IDS.length * sets; i++) {
      expect(
        (await parent.query.childrenBalance({ u64: i }))?.value.unwrap().ok.toString()
      ).to.be.equal("1,0");
    }
    ...
    // deployer transfers all tokens to users
    const manyUsers = createBatchUsers(PARENT_TOKEN_IDS.length * sets);
...
const mintMany = async (contract: Rmrk, signer: KeyringPair, token_ids: ArgumentTypes.Id[]): Promise<void> => {
